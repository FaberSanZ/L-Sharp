    ...
    internal sealed class WindowsRuntimeProjections
    {
        ...
        private static Dictionary<string, ProjectionInfo> Projections
        {
            get
            {
                ...
                Dictionary<string, ProjectionInfo> new_projections = new Dictionary<string, ProjectionInfo> {
        ...
        {
            if (!Projections.TryGetValue(type.Name, out ProjectionInfo info))
            {
        ...
        private static void RedirectInterfaceMethods(TypeReference interfaceType, Collection<MethodDefinition> redirectedMethods)
        {
            ...
            {
                MethodDefinition redirectedMethod = new MethodDefinition(method.Name, MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.Final | MethodAttributes.NewSlot, typeResolver.Resolve(method.ReturnType))
                {
        ...
        public static void Project(TypeReference type)
        {
            ...
            if (Projections.TryGetValue(type.Name, out ProjectionInfo info) && info.WinRTNamespace == type.Namespace)
            {
        ...
        {
            AssemblyNameReference system_runtime = new AssemblyNameReference("System.Runtime", version);
            AssemblyNameReference system_runtime_interopservices_windowsruntime = new AssemblyNameReference("System.Runtime.InteropServices.WindowsRuntime", version);
            AssemblyNameReference system_objectmodel = new AssemblyNameReference("System.ObjectModel", version);
            AssemblyNameReference system_runtime_windowsruntime = new AssemblyNameReference("System.Runtime.WindowsRuntime", version);
            AssemblyNameReference system_runtime_windowsruntime_ui_xaml = new AssemblyNameReference("System.Runtime.WindowsRuntime.UI.Xaml", version);
            AssemblyNameReference system_numerics_vectors = new AssemblyNameReference("System.Numerics.Vectors", version);
