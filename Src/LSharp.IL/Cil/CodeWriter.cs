...
namespace LSharp.IL.Cil
{
    public sealed class CodeWriter : ByteBuffer
    {
        ...
        {
            if (tiny_method_bodies.TryGetValue(body, out uint existing_rva))
            {
                rva = existing_rva;
                return true;
            }
        ...
        {
            if (stack_sizes != null && stack_sizes.TryGetValue(instruction, out int computed_size))
            {
        ...
        {
            if (standalone_signatures.TryGetValue(signature, out MetadataToken token))
            {
                return token;
            }
